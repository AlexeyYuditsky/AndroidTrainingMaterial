...
@FlowPreview
class MainViewModel(private val usersRepository: UsersRepository) : ViewModel(), UsersAdapter.Listener {
    ...
    init {
        val originUsersFlow = searchBy.asFlow()
            .debounce(500)
            .flatMapLatest { usersRepository.getPagedUsers(it) }
    ...
    private fun merge(users: PagingData<User>, localChanges: OnChange<LocalChanges>): PagingData<UserListItem> {
        return users.map { user ->
            ...
            if (user.id == 1L) {
                Log.d("MyLog", "isInProgress = $isInProgress")
                Log.d("MyLog", "localFavoriteFlag = $localFavoriteFlag")
            }
