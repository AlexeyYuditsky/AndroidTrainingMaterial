package com.alexeyyuditsky.test.foundation.views.activity
...
import com.alexeyyuditsky.test.foundation.sideeffects.SideEffectImplementationsHolder
...
class ActivityDelegate(
    ...
    fun onActivityResult(requestCode: Int, responseCode: Int, data: Intent?) {
        implementersHolder.implementations.forEach {
            it.onActivityResult(
                requestCode,
                responseCode,
                data
    ...
    private fun setupSideEffectImplementer(plugin: SideEffectPlugin<*, *>) {
        implementersHolder.putWithPlugin(
            plugin,
