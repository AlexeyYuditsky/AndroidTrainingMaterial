...
import com.alexeyyuditsky.test.databinding.FragmentSecondBBinding
import kotlin.random.Random

class SecondBFragment : Fragment(R.layout.fragment_second_b) {

    private lateinit var binding: FragmentSecondBBinding

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding = FragmentSecondBBinding.bind(view)
        ...
        binding.sendMessageNextButton.setOnClickListener {
            ...
            findNavController().navigate(
                R.id.action_secondBFragment_to_thirdFragment, bundleOf(
                    ThirdFragment.ARG_COLOR to color,
                    ThirdFragment.ARG_TEXT_VIEW to R.string.third_screen,
                    ThirdFragment.ARG_TEXT_TOAST to R.string.second_b_screen,
                )
            )
        }
        binding.sendMessageBackButton.setOnClickListener {
            parentFragmentManager.setFragmentResult(
                MainFragment.REQUEST_KEY, bundleOf(
                    MainFragment.EXTRA_TEXT to R.string.second_b_screen
                )
            )
            findNavController().popBackStack()
        }
